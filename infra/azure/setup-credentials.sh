...
# Build the AZURE_CREDENTIALS JSON (required format for azure/login@v2 with creds)
AZURE_CREDENTIALS="{\"clientId\":\"${CLIENT_ID}\",\"clientSecret\":\"${CLIENT_SECRET}\",\"subscriptionId\":\"${SUBSCRIPTION_ID}\",\"tenantId\":\"${TENANT_ID}\"}"

echo ""
echo ""
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║  GitHub Secret — Add this to your repository                       ║"
...
echo ""
echo "┌──────────────────────────────────────────────────────────────────────┐"
echo "│  ⚠  IMPORTANT: Copy AZURE_CREDENTIALS now!                          │"
echo "│  The clientSecret cannot be retrieved again after this session.      │"
echo "└──────────────────────────────────────────────────────────────────────┘"
echo ""
echo ""
echo "  Quick copy (for GitHub CLI users):"
echo ""
echo "    gh secret set AZURE_CREDENTIALS --body '${AZURE_CREDENTIALS}'"
echo ""
echo ""
