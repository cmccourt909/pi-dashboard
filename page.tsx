...
export default function RoadmapPage() {
  ...
  // Cap end at latest feature end date + 14 days padding so empty future
  // PIs don't compress active work into a tiny slice of the timeline
  const featureEndMs = data.features
    .map(f => f.planned_end)
    .filter(Boolean)
    .map(d => new Date(d as string).getTime());
  const rawEnd = featureEndMs.length
    ? Math.max(...featureEndMs)
    : Math.max(...allDates.map(d => d.getTime()));
  const timelineEnd = new Date(rawEnd + 14 * 86400000);
