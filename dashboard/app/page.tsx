...
function Stat({ label, value, accent }) {
  return (
    <div>
      ...
      <p style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 700, color: accent ?? "var(--text-primary)", lineHeight: 1 }}>
        {value}
      </p>
    </div>
  );
  ...
  return (
    <div style={{ background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: 4, overflow: "hidden" }}>
      <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontFamily: "var(--font-display)", fontSize: 16, fontWeight: 700 }}>RISK FINDINGS</span>
        ...
        {findings.map((f, i) => (
          <div key={f.id ?? i} style={{ padding: "14px 20px", borderBottom: i < findings.length - 1 ? "1px solid var(--border)" : "none", borderLeft: "3px solid " + severityColor(f.severity) }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 4 }}>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, fontWeight: 700, color: severityColor(f.severity), textTransform: "uppercase", letterSpacing: "0.12em" }}>{f.severity}</span>
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--text-muted)", letterSpacing: "0.08em" }}>{f.category}</span>
            </div>
            <p style={{ fontSize: 13, fontWeight: 500, color: "var(--text-primary)", marginBottom: 4 }}>{f.title}</p>
            <p style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.5 }}>{f.description}</p>
            <p style={{ fontSize: 11, color: "var(--status-warning)", marginTop: 6 }}>{"→"} {f.recommendation}</p>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--text-muted)", marginTop: 4 }}>{f.affected_entity}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <>
      <div style={{ marginBottom: 32, paddingBottom: 24, borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
        <div>
          <p style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.15em", color: "var(--text-muted)", marginBottom: 6 }}>PROGRAM INCREMENT OVERVIEW</p>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: 36, fontWeight: 700, color: "var(--text-primary)", lineHeight: 1 }}>PI HEALTH TRACKER</h1>
        </div>
